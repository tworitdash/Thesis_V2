...
c_pp = load('Stt_cone_replica_30_conv_2.mat');
SPP = c_pp.STT;
c_pr = load('Str_cone_replica_30_conv_2.mat');
SPR = c_pr.STR;
c_rp = load('Srt_cone_replica_30_conv_2.mat');
SRP = c_rp.SRT;
c_rr = load('Srr_cone_replica_30_conv_2.mat');
SRR = c_rr.SRR;
...
hold on;
plot(F_cst * 1e-9, db(abs(squeeze(s_params_5_cst(3,86, :)))), 'LineWidth', 2); grid on;
hold on;
plot(F_feko * 1e-9, db(abs(squeeze(s_params_5_feko(2, 84, :)))), 'LineWidth', 2); grid on;
