...
plot(x, real(yap), 'Linewidth', 2);
hold on;
plot(x, imag(yap), 'Linewidth', 2);

grid on;
hold on;
plot(x, real(y11./YTE11), 'Linewidth', 2);
hold on;
plot(x, imag(y11./YTE11), 'Linewidth', 2);
...
%     NTE_FEKO(b) = sqrt((pi*(1)/2 .* (xmn(b).^2 - M(b).^2) .* (besselj(1, xmn(b))).^2).^(-1));
    ETE_higher_rho_i = (Dm(1, b)) .* NTE_FEKO(b) .* M(b)./rho .* besselj(M(b), beta_rho(b) .* rho) .* sin(M(b).*ph);
    ETE_higher_ph_i = (Dm(1, b)) .* NTE_FEKO(b) .* beta_rho(b) .* besselj_der(M(b), beta_rho(b) .* rho) .* cos(M(b).*ph);
...
[Eth, Eph] = FF_apertureFSCir3(1:1:length(N)+1, [1, Dm(1, :)], Gamma, theta, phi, F, er, mur, R);
...
% 
[Eth_fund, Eph_fund] = FF_apertureFSCir3(1, 1, 0, theta, phi, F, er, mur, R);
