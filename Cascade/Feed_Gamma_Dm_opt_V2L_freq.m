function [ModeNumberAper, Transmission_sum]  = Feed_Gamma_Dm_opt_V2L_freq(R, L, F, k, er, mur)
...
for i = 1:length(F)
    ...
    if (N(1) == 1) && (length(F) == 1)
    STR_req = STR(1, 1:N(i, end));
    else
    STR_req = squeeze(STR(i, 1:N(i, end), 1:N(i, 1)));
    end
    bp(i).str_req =  STR_req * ar;  %+ squeeze(STT(1, 1:N(end), 1:N(end))) * ap;
    Transmission_sum = bp;
end
