...
data5_cst = read(rfdata.data,'../../../feko/cst/3wg_V2_more_modes.s300p'); % For PC at TU
...
hold on;
plot(F * 1e-9, angle((squeeze(s_params_5_feko(1, 6, :)))) * 180/pi, 'LineWidth', 2); grid on;
...
hold on;
plot(F * 1e-9, db(abs(squeeze(s_params_5_feko(6, 6, :)))), 'LineWidth', 2); grid on;
