...
z = linspace(2e-2, 4.5e-2, 2);

slope = (r1 - r0)/(z(end) - z(1));
...
%         K = beta_z ./ (omega .* mu .* epsilon^2);
        ...
        % K = 1;
%         K = (pi*(1+deltapm)/2 .* (xmn.^2 - m.^2) .* (besselj(m, xmn)).^2).^(-1);
...
%         K = 1;
%         K = (pi*(1+deltapm)/2 .* (xmn).^2 .* (besselj_der(m, xmn)).^2).^(-1);
...
E = sqrt(abs(E_phi).^2 + abs(E_rho).^2);
...
figure;
surface(rho.*cos(phi), rho.*sin(phi), db(abs(E)./(max(max(abs(E)))))); shading flat;
% surface(rho.*cos(phi), rho.*sin(phi), db(abs(E_z))); shading flat;
colormap('jet');